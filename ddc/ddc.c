 ...
 *
 *  Backported changes from https://github.com/kfix/ddcctl
 *
 */
...
bool DDCRead(CGDirectDisplayID displayID, struct DDCReadCommand *read) {
    ...
    for (int i=1; i<=kMaxRequests; i++) {
        ...
        request.minReplyDelay                   = 100; //50 * kMillisecondScale;  // too short can freeze kernel
...
struct DDCDisplay *DDCGetDisplayList(uint32_t *displayCount) {
    struct DDCDisplay *displays;
    ...
    displays = (struct DDCDisplay*)malloc(sizeof(struct DDCDisplay)*num_displays);
    bzero(displays, sizeof(struct DDCDisplay)*num_displays);
    for(int i = 0; i < num_displays; i++)
    {
        CGDirectDisplayID display_id = display_ids[i];
        displays[i].display_id = display_id;
        displays[i].is_main = CGDisplayIsMain(display_id);
        displays[i].is_builtin = CGDisplayIsBuiltin(display_id);

        CGRect bounds = CGDisplayBounds(display_id);
        displays[i].width = bounds.size.width;
        displays[i].height = bounds.size.height;

        struct EDID edid = {};
        if (EDIDTest(display_id, &edid)) {
